	...
	axios({
		...
		.then(function(response) {
			...
			if (response.data.discount <= 0 || response.data.startDayDiscount == null || response.data.endDayDiscount == null) {
				priceDiscount = `<h3 class="product-price"><span class="new-price">` + formatVND(response.data.price, 0, 'POINT', 0, 'POINT') + 'VND' + `</span></h3>`;
			} else {
				if (now >= startDate && now <= endDate) {
					priceDiscount = `<h3 class="product-price"><span class="new-price">`
						...
						`</span><span class="old-price"><del>` + formatVND(response.data.price, 0, 'POINT', 0, 'POINT')+ 'VND' +`<del></span></h3>`;
				} else {
					priceDiscount = `<h3 class="product-price"><span class="new-price">` + formatVND(response.data.price, 0, 'POINT', 0, 'POINT') + 'VND' +`</span></h3>`;
				}
			}
