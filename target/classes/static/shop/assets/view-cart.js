...
function updateValueDisplay(id) {
	...
	axios.post(baseUrl + '/beauty/cart/update?id=' + id + "&sst=" + qty)
		.then(function(response) {
			...
			document.getElementById('viewTotailPrice' + id).innerHTML = formatVND(price, 0, 'POINT', 0, 'POINT') + 'VND' ;
		})
	...
	axios.get(baseUrl + '/beauty/cart/totail-cart')
		.then(function(response) {
			...
			document.getElementById('totailViewCart').innerHTML = formatVND(totailCart, 0, 'POINT', 0, 'POINT') + 'VND';
			...
			document.getElementById('totailViewOrder').innerHTML = formatVND(totailCart, 0, 'POINT', 0, 'POINT') + 'VND' ;
		})
}
...
function handleShipingClick() {
	axios.get(baseUrl + '/beauty/cart/totail-cart')
		.then(function(response) {
			...
			document.getElementById('totailViewOrder').innerHTML = formatVND(totailViewOrder, 0, 'POINT', 0, 'POINT') + 'VND';
		});
}